...
![Network Traffic Analysis Header](./header_image.png)
...
1. **Initialize Structure**: The newly allocated memory was zeroed out using `memset`.

1. **Process Packet**: nDPI would then classify the packet, treating it as part of a potentially new, isolated flow.

1. **Deallocate Memory**: After processing, the `ndpi_flow_struct` was immediately freed using `ndpi_flow_free`.
...
1. **Creating the Flow Table**: A global `rte_hash` table, `flow_table`, was initialized during application startup. This table is designed to store `flow_key`s and associate them with `flow_value` structures, which in turn hold the `ndpi_flow_struct` and other flow-specific metadata.

1. **Intelligent Packet Processing**: Now, when a packet arrives:
...
**Author**: Open Source Contributor (RAJA MUHAMMAD AWAIS)

**References**:

1. [nDPI packet processing performance question · Issue #2784 - GitHub](https://github.com/ntop/nDPI/issues/2784)

1. [Using nDPI over DPDK to Classify and Block Unwanted Network Traffic - Luca Deri, ntop (PDF)](http://luca.ntop.org/gr2021/altre_slides/nDPI_DPDK_122018.pdf)

1. [2. Hash Library - DPDK Documentation](https://doc.dpdk.org/guides/prog_guide/hash_lib.html)

1. [A Hardware-Accelerated Intrusion Prevention System for Attack Mitigation Using DPUs (IEEE INFOCOM 2025)](https://ieeexplore.ieee.org/abstract/document/11152916/)